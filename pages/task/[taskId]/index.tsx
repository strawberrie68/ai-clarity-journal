...
const SpecificTask = () => {
    ...
    const getTask = async (taskId: string) => {
        try {
            ...
            form.reset({
                ...
                goalId: taskData.goalId ? taskData.goalId.toString() : "",
            });

            setColor(taskData.color || "#ffffff");
    ...
    const updateTask = async (formValue: TaskProps) => {
        if (!userId) {
        ...
        try {
            const updateData = {
                ...formValue,
                userId: userId,
                ...(formValue.goalId && formValue.goalId !== "" && { goalId: formValue.goalId })
            };
    ...
    const updateGoal = async (goalId: string | Types.ObjectId, taskId: string | Types.ObjectId) => {
        if (!goalId || !taskId) {
            console.error("Missing goalId or taskId");
            return;
        }

        const goal = goals.find((g) => getIdString(g._id) === goalId.toString());
        ...
        const taskExists = goal.tasks.some(t => getIdString(t) === taskId.toString());
        if (taskExists) {
            return;
        }
    ...
    const getGoalIdString = (id: string | Types.ObjectId): string => {
    ...
    const onSubmit = async (values: z.infer<typeof taskFormSchema>) => {
        ...
        try {
            ...
            if (values.goalId && (!task?.goalId || values.goalId !== task.goalId.toString())) {
                await updateGoal(values.goalId, taskId as string);
            }
            ...
            setAlert({
                message: error instanceof Error ? error.message : 'Failed to update task. Please try again.',
                type: 'error',
                visible: true
            });
        } finally {
    ...
    return (
        <div className="bg-white mx-6 mt-10 pb-4 lg:max-w-screen-md lg:mx-auto">
            ...
            <p className="mt-4 mb-8 text-lg">Edit task to your to do list</p>
            {alert.visible &&
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        ...
                                        >
                                            ...
                                            {goals.map((goal) => (
                                                <option
                                                    key={getGoalIdString(goal._id)}
                                                    value={getGoalIdString(goal._id)}
                                                >
                                                    {goal.goalName}
                                                </option>
                                            ))}
